...
function address_transform($address) {
...
function get_filename($address) {
    ...
    $filename = str_replace( '№', 'nomer', $filename );
...
{    
    ...
    if (!empty($pos)) {
        ...
        $region = $parse->GeoObjectCollection->featureMember->GeoObject->metaDataProperty->GeocoderMetaData->AddressDetails->Country->AdministrativeArea->AdministrativeAreaName;
        $region = (string)$region;
        
        return array($lng, $lat, $adr, $region);
    }
...
function pagination($url, $num, $col, $page) {
    ...
    $thisPage = '<li class="page-item disabled"><a class="page-link" href="#">'.$page.'</a></li>';
    $totalpage = '<li class="page-item"><a class="page-link" href="'.DOMAIN.$url.'/'.$total.'">'.$total.'</a></li>';
    
    if ($page >= 2) {
      $pervpage = '<li class="page-item"><a class="page-link" href="' . DOMAIN . $url . '">1</a></li>';
    }
        
    else {
      $pervpage ='<li class="page-item disabled"><a class="page-link" href="#">1</a></li>';
      $thisPage = null;
    }
    ...
    if ($page == $total) {
      $totalpage = null;
        ...
        for ($i = 1; $i <= 3; $i++) {
            if ($page - $i > 1) {
                 $pageNav[$i] = '<li class="page-item"><a class="page-link" href="'.DOMAIN.$url.'/'.($page - $i).'">'.($page - $i).'</a></li>';
            }
               
            if ($page + $i < $total) {
                $pageNav2[$i] = '<li class="page-item"><a class="page-link" href="'.DOMAIN.$url.'/'.($page + $i).'">'.($page + $i).'</a></li>';;
            }
        }

    if ($total > 1) {
        $nav .= '<nav aria-label=""><ul class="pagination">';
        $nav .= $pervpage . @$pageNav[3] . @$pageNav[2] . @$pageNav[1] . $thisPage . @$pageNav2[1] . @$pageNav2[2] . @$pageNav2[3] . $totalpage;
        $nav .= '</ul></nav>';
    }
...
function phone_transform($phone) {
    
    $phone = clearData($phone,'int');
    
    if (mb_strlen($phone) == 11) {
        
        // проверяем первую цифру телефона
        if ($phone[0] == 8) {
